...
class FpsLoss(TorchLoss, config_name="fps"):
    ...
    def forward(self, inputs):
        fst_embeddings = inputs[self._fst_embeddings_prefix]
        snd_embeddings = inputs[self._snd_embeddings_prefix]
        ...
        loss = self._loss_function(logits, labels) / 2
...
class SamplesSoftmaxLoss(TorchLoss, config_name="sampled_softmax"):
    ...
    def forward(self, inputs):
        queries_embeddings = inputs[self._queries_prefix]
        positive_embeddings = inputs[self._positive_prefix]
        negative_embeddings = inputs[self._negative_prefix]
        ...
        if negative_embeddings.dim() == 2:
            ...
            assert negative_embeddings.dim() == 3
            negative_scores = torch.einsum(
                "bd,bnd->bn",
                queries_embeddings,
                negative_embeddings,
...
class MCLSRLoss(TorchLoss, config_name="mclsr"):
    ...
    def forward(self, inputs):
        all_scores = inputs[self._all_scores_prefix]
        mask = inputs[self._mask_prefix]
        ...
        positive_scores = all_scores[positive_mask]
